...
export const updateApplication = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const { interviewScheduledAt } = req.body;
    ...
    const normalizedStatus =
      typeof status === "string" && status.trim().length > 0
        ? (statusMap[status.trim().toLowerCase()] as
            | "Applied"
            | "Reviewed"
            | "Shortlisted"
            | "Interview"
    ...
    (application as any).status = normalizedStatus || application.status;
    (application as any).interviewScheduledAt =
      normalizedStatus === "Interview" ? normalizedInterviewScheduledAt : undefined;
    await application.save();
      ...
      if (candidateUser?.email) {
        try {
          await sendApplicationStatusUpdatedEmail({
            to: candidateUser.email,
            candidateName: candidateUser.name,
            status: (normalizedStatus || (application as any).status) as any,
            jobTitle: (application as any).job?.title,
            interviewScheduledAt: normalizedStatus === "Interview" ? normalizedInterviewScheduledAt : undefined,
          });
        } catch (mailError: any) {
    ...
    if (candidateId) {
      await createNotification({
        ...
        message:
          normalizedStatus === "Interview" && normalizedInterviewScheduledAt
            ? `Your interview for ${(application as any).job?.title} has been scheduled for ${normalizedInterviewScheduledAt}.`
            : `Your application status has been updated to ${String((application as any).status)}.`,
        relatedId: (application as any)._id,
      });
    }
